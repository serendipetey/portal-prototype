// portal-prototype/src/components/ButtonFixed.tsx
...
const getButtonStyles = (variant: string, size: string) => {
  const styles: React.CSSProperties = {
    // Typography - Always from CSS custom properties
...
// Create CSS for hover states (injected dynamically)
const createHoverCSS = () => {
  if (typeof document === "undefined") return;

  // Check if styles already exist
  if (document.getElementById("dynamic-button-hover-styles")) return;

  const style = document.createElement("style");
  style.id = "dynamic-button-hover-styles";
  style.textContent = `
    .btn-fixed[data-variant="primary"]:hover {
      background-color: var(--button-primary-bg-hover, #0a2d54) !important;
    }
    .btn-fixed[data-variant="outline"]:hover {
...
const ButtonFixed = React.forwardRef<HTMLButtonElement, ButtonFixedProps>(
  (
    {
      className,
      variant = "primary",
    ...
    // Combine styles: component styles + user styles
    ...
    return (
      <button
        className={cn(
          ...
          "cursor-pointer select-none btn-fixed",
          className
        )}
